/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parser.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tcunha <tcunha@student.42berlin.de>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/03/10 21:04:04 by tcunha            #+#    #+#             */
/*   Updated: 2026/04/06 13:11:52 by tcunha           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"
#include "parser_internal.h"
#include "utils.h"
#include <fcntl.h>
#include <unistd.h>

static int	validate_paths(t_map *map)
{
	int	fd;

	fd = open(map->tex_east, O_RDONLY);
	if (fd == -1)
		return (print_error("@validate_paths", "Failed to open tex_east"), 1);
	close(fd);
	fd = open(map->tex_west, O_RDONLY);
	if (fd == -1)
		return (print_error("@validate_paths", "Failed to open tex_west"), 1);
	close(fd);
	fd = open(map->tex_north, O_RDONLY);
	if (fd == -1)
		return (print_error("@validate_paths", "Failed to open tex_north"), 1);
	close(fd);
	fd = open(map->tex_south, O_RDONLY);
	if (fd == -1)
		return (print_error("@validate_paths", "Failed to open tex_south"), 1);
	close(fd);
	return (0);
}

static int	parser_validate_input(int argc, char *file)
{
	char	*suffix;

	if (argc != 2)
		return (print_error("@parser_validate_input", "Missing .cub"), 1);
	suffix = ft_strnstr(file, ".cub", ft_strlen(file));
	if (!suffix || suffix[4] != '\0')
		return (print_error("@parser_validate_input", "Wrong file type"), 1);
	return (0);
}

int	parser(int argc, char *file, t_map *map)
{
	int	fd;

	if (parser_validate_input(argc, file))
		return (1);
	fd = open(file, O_RDONLY);
	if (fd == -1)
		return (print_error("@parser", "Failed to open .cub"), 1);
	if (parser_textures(map, fd))
		return (close(fd), 1);
	if (validate_paths(map))
		return (close(fd), 1);
	if (parser_map(map, fd))
		return (close(fd), 1);
	close(fd);
	if (parser_validate_grid(map))
		return (1);
	return (0);
}
