...
/*   Updated: 2026/04/06 20:53:11 by tcunha           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_texture *select_texture(t_model *model, t_ray *ray, t_game_player *player)
{
...
int	render(t_game *game, t_ray *ray, int x)
{
	t_texture	*texture;

	texture = select_texture(&game->model, ray, &game->player);
	render_ceiling(&game->model, game->img, x, ray->draw_start);
	//render_wall();
	render_floor(&game->model, game->img, x, ray->draw_end);
	return (0);
}
