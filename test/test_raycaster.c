...
/*   Updated: 2026/04/09 00:30:27 by pecavalc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void init_test_game(t_game *game)
{
...
int main(void)
{
	...
	init_test_game(&game);
	if (!game.model.grid)
		return (fprintf(stderr, "FAIL: grid allocation failed\n"), 1);
	rays = cast_rays(&game);
	if (!rays)
		return (free_grid(game.model.grid), fprintf(stderr, "FAIL: cast_rays returned NULL\n"), 1);
	print_rays(rays, game.mlx.width);
	failed = 0;
	i = 0;
	while (i < game.mlx.width)
	{
		failed |= assert_int_eq("screen_x", rays[i].screen_x, i);
		if (rays[i].perp_wall_dist <= 0.0)
		{
			fprintf(stderr, "FAIL: ray %d has non-positive distance %.6f\n", i, rays[i].perp_wall_dist);
			failed = 1;
		}
		if (rays[i].wall_dir != WEST)
		{
